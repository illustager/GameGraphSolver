...
在本项目的语境下，我们称当前局面为 $x$，指本回合会行动玩家将从 $x$ 开始报数。规定游戏终局为 $M+1$。局面 $x$ 的所有后继局面是 $\left\{y|y = x+k,k \in [1,N], y \leq M+1\right\}$。显然，这是一个 DAG，因此每种局面一定是 P-Position 或 N-Position；具体地：若 $(M+1-x) \bmod (N+1) = 0$，则 $x$ 是 P-Position；否则 $x$ 是 N-Position。
